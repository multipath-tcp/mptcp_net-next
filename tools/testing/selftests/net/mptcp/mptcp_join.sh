...
pm_nl_set_limits()
{
	...
	if [ $ip_mptcp -eq 1 ]; then
		ip -n $ns mptcp limits set add_addr_accepted $nr_addrs subflows $nr_subflows
	else
		ip netns exec $ns ./pm_nl_ctl limits $nr_addrs $nr_subflows
	fi
}
...
do_transfer()
{
	...
	if [ $addr_nr_ns1 -gt 0 ]; then
		...
		if [ $rm_nr_ns1 -lt 8 ]; then
			counter=0
			pm_nl_show_endpoint ${listener_ns} | while read line; do
				local arr=($line)
				local nr=0
	...
	if [ $addr_nr_ns2 -gt 0 ]; then
		...
		if [ $rm_nr_ns2 -lt 8 ]; then
			counter=0
			pm_nl_show_endpoint ${connector_ns} | while read line; do
				local arr=($line)
				local nr=0
	...
	if [ ! -z $bkup ]; then
		...
		for netns in "$ns1" "$ns2"; do
			pm_nl_show_endpoint $netns | while read line; do
				...
				# 'ip mptcp' shows the address at the front
				if [ $ip_mptcp -eq 1 ]; then
					pos=0
				fi
				addr=${arr[$pos]}
