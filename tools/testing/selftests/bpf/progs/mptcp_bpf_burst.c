...
extern bool bpf_sk_stream_memory_free(struct mptcp_subflow_context *subflow) __ksym;
			      ...
			      struct mptcp_sched_data *data)
{
	...
	subflow = bpf_mptcp_subflow_ctx(ssk, data);
	if (!bpf_sk_stream_memory_free(subflow))
		return -1;
