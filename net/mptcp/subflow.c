				...
				struct sk_buff *skb)
{
	...
	if (rx_opt.mp_capable && listener->request_mptcp) {
		subflow_req->mp_capable = 1;
		if (rx_opt.version >= listener->version)
			subflow_req->version = listener->version;
		else
			subflow_req->version = rx_opt.version;
		if ((rx_opt.flags & MPTCP_CAP_CHECKSUM_REQD) ||
		    listener->checksum)
			subflow_req->checksum = 1;
		subflow_req->remote_key = rx_opt.sndr_key;
